...
add_action( 'widgets_init', 'invicta_register_popular_posts_widget' );

function invicta_register_popular_posts_widget() {
	register_widget( 'invicta_popular_posts_widget' );
}
...
class invicta_popular_posts_widget extends WP_Widget {
	
	function invicta_popular_posts_widget() {
		
		$args = array(
			'classname'		=> 'widget_invicta_popular_posts',
			'description'	=> __( 'A widget to display popular post entries', 'invicta_dictionary' )
		);
		
		$this->WP_Widget( 'invicta_popular_posts_widget', THEMENAME . ' ' . __( 'Popular Posts', 'invicta_dictionary' ), $args );
	}
